        ...
        <form action="{{ route('customers.store') }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" id="balance" name="balance" class="form-control" value="{{ old('balance', 0) }}">
                    @error('balance')
                        <div class="text-danger small">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="prepaid_status" name="prepaid_status" class="form-control" required>
                        <option value="postpaid" {{ old('prepaid_status') == 'postpaid' ? 'selected' : '' }}>Postpaid</option>
                        <option value="prepaid" {{ old('prepaid_status') == 'prepaid' ? 'selected' : '' }}>Prepaid</option>
                    </select>
